...
class Budget:
    ...
    def retrieval_params(self, *, base_k: int = 20, base_ef: int = 64,
        ...
        The ladder is ordered by how much quality each step costs:
          >60ms  full quality
          >35ms  drop ef_search (recall dips a little, latency halves)
          >20ms  cut k as well (less for MMR to work with)
          >8ms   sparse-only (BM25 needs no embedding forward pass)
          <=8ms  nothing survives - caller must refuse rather than guess
        """
        r = self.spendable_ms
        if r > 60:
            return {"k": base_k, "ef_search": base_ef, "mode": "hybrid"}
        if r > 35:
            self.note(stage, "ef_search->32")
            return {"k": base_k, "ef_search": 32, "mode": "hybrid"}
        if r > 20:
